...
include "connect.php";
$id=$_GET["id"];
$result=mysql_query("select *from remaildetail where id=$id")or die ("not Work");
...
body{
        ...
        echo "<h17><a href='forward.php?id=$row[id]'>Forward</a>"; 
					    echo "<a href='replay.php?id=$row[id]'>Replay</a></h17>";						
 ...
 <tr><td align="center">subject</td><td align="left" bgcolor="#cccccc"  height="50px"><?PHP 
             echo $row['email_subject'];?></td></tr>
			</tr>							 
							 <tr><td align="top" > message</td> <td  height="100px"  align="top" bgcolor="#e7e2e2"> <?PHP  echo $row['email_message'];?>
            </td></tr>
				...
				<?Php            if($row['name']==NULL) { } else {?> <?PHP echo $row['name'];?><a href="<?PHP echo $row['name'];?>">open</a><?PHP  }  ?>
                   ...
                   ?>   <tr align="right"  height="80px" width="800px"><td></td><td><?PHP 
					    ...
					    echo "<a href='save.php?id=$row[id]'>Save</a></div>"; 
